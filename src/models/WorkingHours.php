...
class WorkingHours extends Model {
  ...
  public function innout($time) {
    ...
    if(!$timeColumn) {
      throw new AppException("Você já bateu todos os pontos para esse dia");
    }
  ...
  public static function getWorkedTimeInMonth($yearAndMonth) {
    $starDate = (new DateTime("{$yearAndMonth}-1"))->format('Y-m-d');
    $endDate  = getLastDayOfMonth($yearAndMonth)->format('Y-m-d');

    $result = static::getResultSetFromSelect([
      'raw' => "work_date BETWEEN '{$starDate}' AND '{$endDate}'"
