...
class User extends Model {
  ...
  private function validate() {
    ...
    if(!$this->email) {
      ...
      $errors['email'] = 'O endereço de email inválido';
    }

    if(!$this->start_date) {
      $errors['start_date'] = 'É necessário informar uma data de admissão';
    } elseif(!DateTime::createFromFormat('Y-m-d', $this->start_date)) {
      $errors['start_date'] = 'Data de admissão deve serguir o padrão dd/mm/aaaa.';
    }
    
   
    if($this->end_date && !DateTime::createFromFormat('Y-m-d', $this->end_date)) {
      $errors['end_date'] = 'Data de desligamento deve serguir o padrão dd/mm/aaaa.';
    }
    ...
    if(!$this->confirm_password) {
      $errors['confirm_password'] = 'A confirmação senha é um campo obrigatório';
    }
